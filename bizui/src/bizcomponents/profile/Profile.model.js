...
export default {
  ...
  effects: {
    ...
    *addShippingAddress({ payload }, { call, put }) {
      const userContext = null
      const {ProfileService} = GlobalComponents;

      const { id, role, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(ProfileService.addShippingAddress, id, parameters)
      if (hasError(data)) {
      ...
      const location = { pathname: `/profile/${id}/list/\ShippingAddressList/Shipping Address+${appLocaleName(userContext,'List')}`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateShippingAddress({ payload }, { call, put }) {
      const userContext = null
      const {ProfileService} = GlobalComponents;      
      const { id, type, parameters, continueNext, selectedRows, currentUpdateIndex } = payload
      console.log('get form parameters', parameters)
      const data = yield call(ProfileService.updateShippingAddress, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
      ...
      const location = { pathname: `/profile/${id}/list/\ShippingAddressList/Shipping Address列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
    *gotoNextShippingAddressUpdateRow({ payload }, { call, put }) {
      const { id, type, parameters, continueNext, selectedRows, currentUpdateIndex } = payload
      const newPlayload = { ...payload, selectedRows, currentUpdateIndex }
      yield put({ type: 'updateState', payload: newPlayload })
    },
    *removeShippingAddressList({ payload }, { call, put }) {
     const userContext = null
      const {ProfileService} = GlobalComponents; 
      const { id, role, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(ProfileService.removeShippingAddressList, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
